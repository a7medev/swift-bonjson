...
BONJSON supports the following types:
...
**Note**: Binary data is an extension beyond standard JSON types.
...
| 65        | Arbitrary length data        | Binary    | [Binary Data](#binary-data)                |
| 66 - 67   |                              |           | RESERVED                                   |
...
Binary data allows encoding of raw byte sequences. Unlike [strings](#strings), binary data is not subject to UTF-8 validation.

Binary data begins with the [type code](#type-codes) (`0x65`), followed by a [length field](#length-field), and then the raw bytes:

    0x65 [length] [bytes]

The `length field` specifies the number of bytes of binary data that follow. The binary data bytes are written directly without any encoding or transformation.

**Note**: Binary data does not support chunking. The entire binary data blob must be written with a single length field.

**Examples**:

    65 01                         // Empty binary data (0 bytes)
    65 09 01 02 03 04             // 4 bytes: [0x01, 0x02, 0x03, 0x04]
    65 11 48 65 6c 6c 6f 21 00 ff // 8 bytes: [0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x21, 0x00, 0xff]
...
value             = array | object | number | boolean | string | binary | null;
...
binary                = u8(0x65) & binary_length(var(len, ~)) & sized(len*8, u8(~)*);
binary_length(l)      = length(l * 2);  # continuation bit always 0 for binary
